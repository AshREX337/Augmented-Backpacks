...
public class TrashMenu extends AugmentSettingsMenu {
    ...
    public TrashMenu(PopupMenuHandler handler, AugmentHolder<TrashAugment> holder) {
        super(handler, menu -> {
            ...
            ItemGrid<TrashAugment> grid = ItemGrid.builder(holder::get, holder::update)
            ...
            layout.addChild(createOption(MODE_LABEL, MODE_LABEL_TOOLTIP, BackpackButtons.values(() -> holder.get().invMode(), value -> holder.update(holder.get().toggleMode(value)), filterMode -> {}).setSize(60, 18).build(), divider.getWidth()));
            return layout;
        });
    }
}
